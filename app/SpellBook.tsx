...
export function SpellBook({ chapters }: { chapters: Chapter[] }) {
  ...
  useEffect(() => { chapters.forEach((_, i) => { const image = new Image(); image.src = asset(`/assets/castle/spell-chapter-${i + 1}.webp`); }); }, [chapters]);
  ...
  return <div className={`spell-reading ${turn ? "is-turning" : ""}`}>
    ...
    <img className="spell-illustration" src={asset(`/assets/castle/spell-chapter-${index + 1}.webp`)} alt={descriptions[index]} />
    {turn && <div className={`spell-turn-sheet ${turn.to < turn.from ? "is-backward" : ""}`} aria-hidden="true"><div className="spell-sheet-front"><img src={asset(`/assets/castle/spell-chapter-${turn.from + 1}.webp`)} alt="" /></div><div className="spell-sheet-back"><span>{chapters[turn.to].title}</span></div></div>}
